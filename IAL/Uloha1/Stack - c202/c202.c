...
*/
	if(s == NULL)
		...
		s->top = -1;	//nastavi vrchol zasobnika na -1
...
*/
	return (s->top == -1) ? 1 : 0; //ak je zasobnik prazdny vrati 1, inak 0
}
...
*/
	//if(s->top == STACK_SIZE - 1) : {return 1;} ? {return 0;};
	return (s->top == STACK_SIZE - 1) ? 1 : 0; //ak je zasobnik plny vrati 1, inak 0
...
*/
	if(stackEmpty(s))
		...
		*c = s->arr[s->top]; //na adresu c preda int hodnotu z vrcholu zasobnika
...
if(!(stackEmpty(s)))
	s->top--; //odstrani prvek zo zasbonika
}
...
*/
	if(stackFull(s))
		...
		s->arr[++(s->top)] = c; //prida prvok na vrchol zasobnika
