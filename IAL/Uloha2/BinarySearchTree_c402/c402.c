...
**/
	...
	if(!new)	return;	//akk sa alokacia nepodarila
	new->Cont = Content;
	new->LPtr = NULL;
	new->RPtr = NULL;
	...
	else if((*RootPtr)->Cont != new->Cont) //ak obsah noveho uzlu sa nerovna obsahu korena
	{
		tBTNodePtr tmp, last;
		tmp = *RootPtr;
		while(tmp)	//hladaj kde treba umiestnit novy uzol
		{
			if(new->Cont < tmp->Cont)
			{
				last = tmp;
				tmp = tmp->LPtr;
			}
			else if(new->Cont > tmp->Cont)
			{
				last = tmp;
				tmp = tmp->RPtr;
			}
		...
		if(new->Cont < last->Cont) //daj ho nalavo
				last->LPtr = new;
		else if (new->Cont > last->Cont)
				last->RPtr = new;	//daj ho napravo
	}
...
**/
	if(!(*RootPtr)) return;//ak je strom prazdny nerob nic
	tStackP stack;
	SInitP(&stack);
	do{
		if((*RootPtr)) //ak mame koren
		{
			if((*RootPtr)->LPtr)
				SPushP(&stack, (*RootPtr)->LPtr); //ak ma lavy
	...
	while((*RootPtr) || (!SEmptyP(&stack))); //kym nie je strom prazdny a kym je nieco na stacku
}	
