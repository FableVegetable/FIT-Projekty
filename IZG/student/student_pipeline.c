...
void const *gpu_computeVertexAttributeDataPointer(
  ...
  VertexIndex index = gl_VertexID; 

  if(head->enabled) 
    return (char *)head->buffer + head->offset + head->stride * index;  
  else
    return NULL;
}
...
void gpu_runVertexPuller(GPUVertexPullerOutput *const              output,
  ...
  VertexIndex vindex = gpu_computeGLVertexID(puller->indices, vertexShaderInvocation);

  for(int i=0; i < MAX_ATTRIBUTES; i++)
    output->attributes[i] = gpu_computeVertexAttributeDataPointer(&(puller->heads[i]), vindex);
}
...
///  - GPUVertexShaderInput()
  ...
  GPUVertexShaderOutput*  shader_out = primitive->vertices;

  for(size_t index = baseVertexShaderInvocation;  index < nofPrimitiveVertices + baseVertexShaderInvocation; index++)
  {
    gpu_runVertexPuller(&puller_out, puller, index);
    shader_in.attributes = &puller_out;
    shader_in.gl_VertexID = gpu_computeGLVertexID(puller->indices, index);
...
void gpu_computeScreenSpaceBarycentrics(
  ...
  coords->data[2] = distanceTo2DLine(&lines[0],pixelCenter) / distanceTo2DLine(&lines[0],&vertices[2]);
}
